...
//    ;
BYTE EEPROMWritePage(WORD addr, BYTE __xdata * ptr, BYTE len)
{
    BYTE i;
    BYTE retval;
...
// Returns 0 on success, 1 on failure
BYTE EEPROMRead(WORD addr, BYTE length, BYTE __xdata *buf)
{
   BYTE i;
...
BYTE EEPROMGetPageSize()
{
      ...
      BYTE __xdata testData[MAX_PAGE_SIZE];
      BYTE __xdata saveData[MAX_PAGE_SIZE];
      BYTE i;
      BYTE retval;
